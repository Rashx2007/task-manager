...
export function dxfToSvg(dxfPath, basePatterns = []) {
  ...
  for (const e of ents) { geoPoints(e, addP); if (isBase(e.layer)) geoPoints(e, addB); }
    ...
    try {
      ...
      switch (e.type) {
        ...
        case 'INSERT': {
          ...
          for (const c of b.entities) {
            if ((c.type === 'TEXT' || c.type === 'MTEXT' || c.type === 'ATTRIB') && c.startPoint) {
              const t = c.type === 'MTEXT' ? cleanMtext(c.text) : String(c.text ?? c.value ?? '').trim();
              if (!t) continue;
              const w = toWorld(c.startPoint.x, c.startPoint.y);
              texts.push({ layer: c.layer && c.layer !== '0' ? c.layer : L, text: t, x: X(w.x), y: Y(w.y), groupId: gid });
            }
          }
        ...
        case 'MTEXT': {
          ...
          if (txt && e.startPoint) texts.push({ layer: e.layer || '0', text: txt, x: X(e.startPoint.x), y: Y(e.startPoint.y), groupId: null }); return `<text data-tag="${esc(txt)}" x="${wx(e.startPoint.x).toFixed(1)}" y="${wy(e.startPoint.y).toFixed(1)}" font-size="11">${esc(txt)}</text>`;
        }
        default: return '';
      }
    } catch { return ''; }
