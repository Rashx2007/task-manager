...
// ✅ تبدیل تاریخ به شمسی برای گزارش‌های Excel (فرمت: 1405/07/03)
export function toShamsiString(d) {
  if (!d) return '';
  const dt = new Date(d);
...
// ✅ تبدیل تاریخ و ساعت به شمسی برای گزارش‌های Excel (فرمت: 1405/07/03 14:30)
export function toShamsiDateTimeString(d) {
  if (!d) return '';
  const dt = new Date(d);
  if (isNaN(dt.getTime())) return '';
  const date = new Intl.DateTimeFormat('fa-IR-u-nu-latn', {
