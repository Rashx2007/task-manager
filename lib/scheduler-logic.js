...
return query(`SELECT DISTINCT TD.TaskID, TD.Priorities, TD.FixedDueTime, TD.Durationtime, TD.DueDateTime, TD.EndDateTime FROM TimeDate_tbl TD LEFT JOIN Tsk_tbl tsk ON TD.TaskID=tsk.TaskID WHERE ((TD.DueDateTime >= ?) OR (TD.DueDateTime <= ? AND TD.EndDateTime > ?)) AND TD.TaskID <> ? AND tsk.Complited = 0 AND TD.FixedDueTime = 0 ORDER BY TD.Priorities, TD.DueDateTime`,
...
const lower = await query(`SELECT TD.TaskID, TD.DueDateTime FROM TimeDate_tbl TD LEFT JOIN Tsk_tbl tsk ON TD.TaskID=tsk.TaskID WHERE TD.Priorities > ? AND TD.TaskID <> ? AND TD.FixedDueTime <> 1 AND tsk.Complited = 0 ORDER BY TD.Priorities, TD.DueDateTime`, [priority, taskId]);
if (lower.length) return new Date(lower[0].DueDateTime);
const last = await query(`SELECT TOP 1 TD.EndDateTime FROM TimeDate_tbl TD LEFT JOIN Tsk_tbl tsk ON TD.TaskID=tsk.TaskID WHERE tsk.Complited = 0 AND TD.FixedDueTime <> 1 ORDER BY TD.Priorities DESC, TD.DueDateTime DESC`);
...
const due = correctDueDateTime(await initializeDueDateTime(taskId, priority), wh);
const end = new Date(due.getTime() + durMs);
return { due, end };
}
export async function sortNonFixedTasks(taskId, insertDue, startCursor) {
const wh = await loadWorkHoursFromSettings();
...
const wh = await loadWorkHoursFromSettings();
const rows = await query(`SELECT TD.TaskID, TD.Priorities, TD.Durationtime FROM TimeDate_tbl TD LEFT JOIN Tsk_tbl tsk ON TD.TaskID=tsk.TaskID WHERE (TD.Priorities <> N'زمان انجام ثابت') AND (tsk.Complited = 0) ORDER BY TD.Priorities, TD.DueDateTime`);
