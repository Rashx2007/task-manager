...
export async function findNonFixedAfter(taskId, due) {
  return query(`SELECT DISTINCT TD.TaskID, TD.Priorities, TD.FixedDueTime, 
    TD.Durationtime, TD.DueDateTime, TD.EndDateTime FROM TimeDate_tbl TD 
    LEFT JOIN Tsk_tbl tsk ON TD.TaskID=tsk.TaskID 
    WHERE (TD.DueDateTime >= ?) AND TD.TaskID <> ? 
    AND tsk.Complited = 0 AND TD.FixedDueTime = 0 
    ORDER BY TD.Priorities, TD.DueDateTime`, [formatSqlDateTime(due), taskId]);
}
