...
import { fmtFaDT, fmtFaDateOnly, toShamsiString, toShamsiDateTimeString } from './shamsi';

// ✅ تبدیل ورودی تاریخ به پارامتر Date امن برای msnodesql (بدون CONVERT متنی)
const dayStart = (v) => {
  const d = new Date(v);
...
export async function getAssetDateTasks(start, end, assetName) {
  const params = [dayStart(start), dayEnd(end)];
  let assetCond = "";
  if (assetName) {
  ...
  return await query(
    `SELECT DISTINCT tsk.TaskID, asset.AssetName, asset.AssetNumber,
     ...
     LEFT JOIN TimeDate_tbl TD ON TD.TaskID = tsk.TaskID
     WHERE (TD.Finish_DateTime >= ?)
       AND (TD.Finish_DateTime <= ?)
       ${assetCond}
