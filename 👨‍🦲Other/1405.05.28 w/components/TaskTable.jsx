...
const assetSpec = (t) =>
  t.AssetName
    ? `${t.AssetName}، قسمت: ${t.Location || '-'} (ساختمان ${t.Building || '-'}، بلوک: ${t.Block || '-'}، طبقه: ${t.Floor ?? '-'}، ورودی: ${t.Entrance || '-'}) شماره: ${t.AssetNumber ?? '-'}`
    : '';
...
export default function TaskTable({ tasks, onRowClick, onComplete, onEdit, onFolder, selectedTask }) {
  ...
  return (
        <div
      className="overflow-auto overscroll-contain rounded-lg shadow-lg"
      style={{ maxHeight: 'calc(100vh - 170px)', minHeight: '300px' }}
    >
      <table className="task-table w-full min-w-[1400px]">
        <thead>
          <tr>
            ...
            <th>زمان شروع</th><th>ضمائم</th><th>عملیات</th>
          </tr>
        </thead>
        <tbody>
          {tasks.map((t, i) => (
            <tr key={t.TaskID} onClick={() => onRowClick(t)} onDoubleClick={() => onEdit && onEdit(t)}
              ...
              <td>{Number(t.Complited) !== 1 && (<button className="btn-success px-2 py-1 text-xs" onClick={(e) => { e.stopPropagation(); onComplete(t.TaskID); }}>اتمام</button>)}</td>
            </tr>
          ))}
        </tbody>
      </table>
