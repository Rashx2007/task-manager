...
function toDateStr(iso, endOfDay = false) {
  const d = new Date(iso);
  if (isNaN(d.getTime())) return null;
  const p = (n) => String(n).padStart(2, '0');
  const base = `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}`;
  return endOfDay ? `${base} 23:59:59` : `${base} 00:00:00`;
}
...
export async function GET(request) {
  try {
    ...
    const s = start ? toDateStr(start) : '1900-01-01 00:00:00';
    const e = end ? toDateStr(end, true) : '2100-12-31 23:59:59';

    let rows = [];

    if (type === 'tasks_list') {
      rows = await query(`SELECT DISTINCT tsk.TaskID, asset.AssetName, asset.AssetNumber, asset.Building, asset.Location,
        asset.Block, asset.Floor, tsk.TaskTtl, tsk.Descriptions, TD.Priorities,
        FORMAT(TD.DueDateTime,'HH:mm  yyyy/MM/dd','fa') AS DueFa,
        FORMAT(TD.Submit_Date,'HH:mm  yyyy/MM/dd','fa') AS SubmitFa, tsk.Complited
        FROM Tsk_tbl tsk
      ...
      rows = await query(`SELECT TD.TaskID, TD.Priorities, TD.Durationtime,
        FORMAT(TD.DueDateTime,'yyyy/MM/dd  HH:mm','fa') AS StartFa,
        FORMAT(TD.EndDateTime,'yyyy/MM/dd  HH:mm','fa') AS EndFa, tsk.Complited
      ...
      rows = await query(`SELECT pur.TaskID, pur.RequestNumber, pur.RegisterNumber, asset.Location,
        tsk.TaskTtl, tsk.Descriptions, FORMAT(pur.RequestDate,'yyyy/MM/dd','fa') AS ReqFa,
        pur.Buyer, pur.Status, FORMAT(pur.FundingDate,'yyyy/MM/dd','fa') AS FundFa,
      ...
      rows = await query(`SELECT pur.TaskID, pur.RequestNumber, pur.RegisterNumber, tsk.TaskTtl, tsk.Descriptions,
        FORMAT(pur.RequestDate,'yyyy/MM/dd','fa') AS ReqFa, pur.Buyer, pur.Status,
        FORMAT(pur.FundingDate,'yyyy/MM/dd','fa') AS FundFa, tsk.Complited
      ...
      rows = await query(`SELECT DISTINCT tsk.TaskID, pr.PersonName AS Functor, asset.AssetName, asset.AssetNumber,
        asset.Building, asset.Location, tsk.TaskTtl, tsk.Descriptions, TD.Priorities,
        FORMAT(TD.DueDateTime,'HH:mm  yyyy/MM/dd','fa') AS DueFa,
        FORMAT(TD.Submit_Date,'HH:mm  yyyy/MM/dd','fa') AS SubmitFa
        FROM Tsk_tbl tsk
        LEFT JOIN Asset_Task_tbl assettsk ON tsk.TaskID = assettsk.TaskID
        LEFT JOIN Asset_2_tbl asset ON assettsk.AssetID = asset.AssetID
        LEFT JOIN TimeDate_tbl TD ON TD.TaskID = tsk.TaskID
        LEFT JOIN ApplicantFunctor_tbl AF ON AF.TaskID = tsk.TaskID
        LEFT JOIN Persons_tbl pr ON AF.FunctorID = CAST(pr.PersonID AS NVARCHAR(20))
        WHERE tsk.Complited < 1
        ORDER BY TD.Priorities, pr.PersonName, TD.DueDateTime`);
    } else if (type === 'daily') {
      rows = await query(`SELECT tsk.TaskID, asset.AssetName, asset.AssetNumber, asset.Building, asset.Location,
        tsk.TaskTtl, FL.Description, CAST(FL.DueDateTime AS DATE) AS DoneDate
        FROM Follow_tbl FL
        LEFT JOIN Tsk_tbl tsk ON FL.TaskID = tsk.TaskID
        LEFT JOIN Asset_Task_tbl assettsk ON tsk.TaskID = assettsk.TaskID
        LEFT JOIN Asset_2_tbl asset ON assettsk.AssetID = asset.AssetID
        WHERE FL.DueDateTime >= ? AND FL.DueDateTime < ?
        ORDER BY tsk.TaskID DESC, FL.DueDateTime`, [s, e]);
    } else if (type === 'considerable') {
      rows = await query(`SELECT DISTINCT tsk.TaskID, tsk.IsConsiderableAction, asset.AssetName, asset.AssetNumber,
        asset.Building, asset.Location, tsk.TaskTtl,
        FORMAT(TD.Submit_Date,'yyyy/MM/dd','fa') AS SubmitFa,
        FORMAT(TD.Finish_DateTime,'yyyy/MM/dd','fa') AS FinishFa, tsk.Complited
        FROM Tsk_tbl tsk
        LEFT JOIN Asset_Task_tbl assettsk ON tsk.TaskID = assettsk.TaskID
        LEFT JOIN Asset_2_tbl asset ON assettsk.AssetID = asset.AssetID
        LEFT JOIN TimeDate_tbl TD ON TD.TaskID = tsk.TaskID
        ...
        ORDER BY TD.Submit_Date`, [s, e]);
    } else if (type === 'all_by_date') {
      rows = await query(`SELECT DISTINCT tsk.TaskID, tsk.IsConsiderableAction, asset.AssetName, asset.AssetNumber,
        asset.Building, asset.Location, tsk.TaskTtl, tsk.Descriptions,
        FORMAT(TD.Submit_Date,'yyyy/MM/dd','fa') AS SubmitFa,
        FORMAT(TD.Finish_DateTime,'HH:mm  yyyy/MM/dd','fa') AS FinishFa, tsk.Complited
        FROM Tsk_tbl tsk
        LEFT JOIN Asset_Task_tbl assettsk ON tsk.TaskID = assettsk.TaskID
        LEFT JOIN Asset_2_tbl asset ON assettsk.AssetID = asset.AssetID
        LEFT JOIN TimeDate_tbl TD ON TD.TaskID = tsk.TaskID
      ...
      rows = await query(`SELECT DISTINCT tsk.TaskID, tsk.IsConsiderableAction, asset.AssetName, asset.AssetNumber,
        ...
        LEFT JOIN Asset_2_tbl asset ON assettsk.AssetID = asset.AssetID
        LEFT JOIN TimeDate_tbl TD ON TD.TaskID = tsk.TaskID
        WHERE tsk.Complited = 1 AND TD.Finish_DateTime >= ? AND TD.Finish_DateTime <= ?
        ORDER BY TD.Submit_Date`, [s, e]);
    }
