...
export default function ReportViewer({ config, onClose }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (config.needsDateRange) {
        if (!start || !end) { showToast('لطفاً بازه زمانی را انتخاب کنید.', 'warn'); setLoading(false); return; }
      ...
      if (config.output === 'text') {
        ...
        showToast('گزارش تولید شد؛ آمادهٔ مشاهده و ذخیره است.', 'success', 2500);
      }
    } catch (e) {
  ...
  const save = async () => {
    ...
    const data = content.kind === 'text'
      ...
      : new Blob([bufferRef.current], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });

    // تلاش برای دیالوگ Save مدرن (showSaveFilePicker)
    if (window.showSaveFilePicker) {
      try {
        const handle = await window.showSaveFilePicker({
          suggestedName: fileName,
          types: [{ description: 'گزارش', accept: { [data.type]: ['.' + fileName.split('.').pop()] } }],
        });
    ...
    <div className="fixed inset-0 bg-black/60 z-[10000] flex items-center justify-center p-4"
      ...
      <div className="bg-[#CCE6DF] rounded-lg shadow-2xl w-[900px] max-w-[95vw] max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-4 border-b border-teal-800/20 flex flex-wrap gap-3 items-end">
          ...
          {content && (
            <>
              <button onClick={save} className="btn-success">💾 ذخیره / دانلود</button>
        ...
        <div className="flex-1 overflow-auto p-4 bg-white">
          {!content && <div className="text-center text-gray-500 py-8">برای مشاهده، ابتدا گزارش را تولید کنید.</div>}
          {content?.kind === 'text' && (
            <pre className="whitespace-pre-wrap font-[Vazirmatn] text-sm leading-relaxed text-gray-800" dir="rtl">
              {content.text}
            </pre>
          )}
          {content?.kind === 'excel' && (
            <div className="text-center py-12">
              ...
              <div className="text-lg font-bold text-teal-900 mb-2">گزارش Excel آماده است</div>
              <div className="text-sm text-gray-600 mb-6">حجم: {(content.size / 1024).toFixed(1)} KB</div>
              <div className="text-xs text-gray-500">برای ذخیره با دیالوگ ویندوز، دکمه «💾 ذخیره / دانلود» را بزنید.</div>
            </div>
          )}
        </div>
      </div>
