...
export default function DwgBrowser({ onClose, onSelect, title = 'انتخاب فایل نقشه (DWG)' }) {
  ...
  const goUp = () => {
    if (!current) return;
    if (isRoot) load('');      // ✅ از ریشهٔ درایو → صفحهٔ انتخاب درایو
    else load(parent);
  };
    ...
    <div className="fixed inset-0 bg-black/50 z-[10001] flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#CCE6DF] rounded-lg shadow-2xl w-[900px] max-w-full max-h-[88vh] flex flex-col">
        ...
        <div className="flex items-center gap-2 p-3">
          ...
          <button className={`px-3 ${view === 'grid' ? 'btn-primary' : 'btn'}`} onClick={() => setView('grid')} title="نمایش شبکه‌ای">▦</button>
        ...
        <div className="flex-1 overflow-auto bg-white mx-3 rounded border p-3" style={{ minHeight: 300 }}>
          ...
          {current && view === 'grid' && (
            <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
              {dirs.map((d) => (
                <button key={'d' + d} className="border rounded p-3 hover:bg-teal-50 flex flex-col items-center gap-2" onClick={() => load(join(d))}>
                  <span className="text-4xl">📁</span>
                  <span className="text-[11px] font-bold break-all text-center">{d}</span>
                </button>
              ))}
              {files.map((f) => (
                <button key={'f' + f} className={`border rounded p-3 flex flex-col items-center gap-2 ${selected === f ? 'bg-yellow-100 ring-2 ring-teal-600' : 'hover:bg-yellow-50'}`} onClick={() => setSelected(f)}>
                  <span className="text-4xl">🗺</span>
                  <span className="text-[11px] font-bold break-all text-center" dir="ltr">{f}</span>
                </button>
              ))}
          ...
          {current && view === 'list' && (
            <div>
              {dirs.map((d) => (
                <div key={'d' + d} className="px-2 py-1 hover:bg-teal-50 cursor-pointer text-sm flex items-center gap-2" onClick={() => load(join(d))}>📁 {d}</div>
              ))}
              {files.map((f) => (
                <div key={'f' + f} className={`px-2 py-1 cursor-pointer text-sm flex items-center gap-2 ${selected === f ? 'bg-yellow-100 font-bold' : 'hover:bg-yellow-50'}`} onClick={() => setSelected(f)}>🗺 <span dir="ltr">{f}</span></div>
              ))}
