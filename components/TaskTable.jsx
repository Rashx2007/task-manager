...
// ✅ خانه‌های ردیف دوم پیش‌نویس (مشخصات مکانی دستگاه)
const PLACE_COLS = [
...
export default function TaskTable({
  ...
  const loadMenuValues = async (col) => {
    ...
    if (draftActive) {
      constraints = {
        ...
        TaskTtl: draft.TaskTtl,
        Priorities: draft.Priorities,
      };
      delete constraints[col.key];
      constraints = Object.fromEntries(
        Object.entries(constraints).filter(([, v]) => norm(v) !== ""),
      );
    }
    try {
  ...
  const checkDeviceExists = async () => {
    ...
    try {
      if (!assetNamesRef.current) {
        ...
        assetNamesRef.current = (d.success ? d.data || [] : []).map((a) =>
          norm(a.AssetName),
        );
      }
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    let av, bv;
    if (sortKey === "status") {
      av = Number(a.Complited);
      bv = Number(b.Complited);
    } else {
      av = a[sortKey];
      bv = b[sortKey];
    }
  ...
  const setFilterAll = (key) =>
    setFilters((prev) => ({ ...prev, [key]: new Set(menuValues || []) }));
  ...
  const hasAnyFilter =
    !draftActive &&
    Object.keys(filters).some((k) => filters[k] && filters[k].size > 0);
  ...
  const draftCell = (key, ph) => (
      ...
      <div className="draft-cell-wrap">
        <input
          ...
          onClick={(e) => {
            ...
            if (col) openMenu(e, col);
          }}
          onBlur={() => {
            if (key === "AssetName") checkDeviceExists();
          }}
        />
        <button
          type="button"
    ...
    // ✅ خانهٔ ردیف دوم پیش‌نویس (بلوک/طبقه/ورودی/سیستم)
  const draftPlaceCell = (col, disabled) => (
    <td className="draft-cell">
      <div className="draft-cell-wrap">
        <input
          className="draft-input"
          value={draft[col.key] != null ? String(draft[col.key]) : ''}
    ...
    <div className="h-full overflow-auto overscroll-contain rounded-lg shadow-lg bg-[#b4a9b0]">
      <table className="task-table w-full min-w-[1300px]">
        <thead>
          <tr>
            {COLUMNS.map((col) => {
              const isFiltered =
                !draftActive && filters[col.key] && filters[col.key].size > 0;
        ...
        <tbody>
          {draftActive && (
            <tr className="draft-row">
              <td>—</td>
              <td>
                <span className="draft-badge">پیش‌نویس</span>
              </td>
            ...
            <tr className="draft-row draft-row-place">
              ...
              {draftPlaceCell(PLACE_COLS[0], !isCentral(draft.Building))}
              {draftPlaceCell(PLACE_COLS[1], !isCentral(draft.Building))}
              {draftPlaceCell(PLACE_COLS[2], !isCentral(draft.Building))}
              {draftPlaceCell(PLACE_COLS[3], false)}
              <td colSpan={8} className="draft-place-note">
                {isCentral(draft.Building)
                  ? '🏢 ساختمان مرکزی: بلوک، طبقه و ورودی را وارد یا از ▾ انتخاب کنید.'
                  : '🏢 ساختمان غیرمرکزی: بلوک «-»، طبقه «0» و ورودی «-» خودکار اعمال شد.'}
              </td>
            </tr>
          )}
              ...
              >
                ...
                {hasAnyFilter && (
                  <button
                    ...
                    className="btn-danger px-2 py-1 text-xs mr-2"
                  >
                    حذف همه فیلترها
                  </button>
                )}
          ...
          {sorted.map((t, i) => (
            <tr
              ...
              className={
                selectedTask?.TaskID === t.TaskID ? "task-row-selected" : ""
              }
            ...
            >
              ...
              <td className="max-w-[220px]">
                <Tip tip={t.TaskTtl} block>
                  <div className="truncate">{t.TaskTtl}</div>
                </Tip>
              </td>
              <td className="max-w-[320px]">
                <Tip tip={t.Descriptions} block>
                  <div className="line-clamp-2">{t.Descriptions || "-"}</div>
                </Tip>
              </td>
              <td>{t.Priorities || "-"}</td>
      ...
      {hasAnyFilter && (
        <div className="filter-summary">
          ...
          {Object.keys(filters)
            .filter((k) => filters[k] && filters[k].size > 0)
            .map((k) => {
              const col = COLUMNS.find((c) => c.key === k);
              return (
                <span key={k} className="filter-chip">
                  {col.label}: {filters[k].size} مورد
                  <button type="button" onClick={() => clearFilter(k)}>
                    ✕
                  </button>
                </span>
              );
              ...
              <div className="col-menu-section">
                <div className="col-menu-title">
                  {menuCol.assignable
                    ...
                    : "این ستون برای پیش‌نویس قابل انتخاب نیست"}
                </div>
                {menuCol.assignable && menuLoading && (
                  <div className="col-menu-empty">در حال خواندن گزینه‌ها…</div>
                )}
                {menuCol.assignable &&
                  !menuLoading &&
                  menuValues &&
                  menuValues.length === 0 && (
                    <div className="col-menu-empty">گزینه‌ای یافت نشد</div>
                  )}
                {menuCol.assignable &&
                  !menuLoading &&
                  menuValues &&
              ...
              <>
                {menuCol.sortable && (
                  <div className="col-menu-section">
                    ...
                    <button
                      type="button"
                      onClick={() => toggleSort(menuCol.key)}
                    >
                      {sortKey === menuCol.key && sortDir === "asc"
                        ? "✓ "
                        : "   "}
                      صعودی
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        setSortKey(menuCol.key);
                        setSortDir("desc");
                      }}
                    >
                      {sortKey === menuCol.key && sortDir === "desc"
                        ? "✓ "
                        : "   "}
                      نزولی
                    </button>
                ...
                {menuCol.filterable && (
                  <div className="col-menu-section">
                    <div className="col-menu-title">
                      فیلتر از کل دیتابیس
                      {menuValues ? ` (${menuValues.length} مقدار)` : ""}
                    </div>
                    <div className="col-menu-actions">
                      <button
                        type="button"
                        onClick={() => setFilterAll(menuCol.key)}
                      >
                        انتخاب همه
                      </button>
                      <button
                        type="button"
                        onClick={() => clearFilter(menuCol.key)}
                      >
                        حذف فیلتر
                      </button>
                    </div>
