...
export default function TaskTable({
  ...
  const loadMenuValues = async (col) => {
    ...
    if (draftActive) {
      ...
      constraints = Object.fromEntries(
        Object.entries(constraints).filter(([, v]) => {
  ...
  const hasAnyFilter =
    !draftActive && Object.values(filters).some((s) => s != null);
  ...
  const draftPlaceCell = (col) => {
    const disabled =
      col.key === "Block" || col.key === "Entrance" ? !centralDraft : false;
    return (
      <td className="draft-cell">
        <div className="draft-cell-wrap">
          <input
        ...
        <tbody>
          ...
          {draftActive && (
            <tr className="draft-row draft-row-place">
              <td colSpan={2}>
                <span className="draft-badge">سیستم دستگاه</span>
              </td>
            ...
            >
              ...
              <td>
                <div className="row-actions">
                  ...
                  <button
                    type="button"
                    className="row-action row-action-copy"
                    title="کپی (ایجاد کار جدید برابر با این کار)"
                    onClick={(e) => {
                      e.stopPropagation();
                      onCopy && onCopy(t);
                    }}
            ...
            {draftActive ? (
              <div className="col-menu-section">
                ...
                {menuCol.assignable ? (
                  <>
                    ...
                    {!menuLoading && shownValues.length === 0 && (
                  ...
                  <div className="col-menu-empty">
                    الویت پس از ثبت کار (تکمیل ثبت) تنظیم می‌شود.
                  </div>
                )}
              </div>
            ) : (
              <>
                ...
                {menuCol.filterable && (
                  <div className="col-menu-section">
                    <div className="col-menu-title">
                      فیلتر از کل دیتابیس ({menuValues ? menuValues.length : 0}{" "}
                      مقدار)
                    </div>
                    <input
                      className="col-menu-search"
                    ...
                    <label className="col-menu-item col-menu-selectall">
                      <input
                        type="checkbox"
                        checked={allChecked(menuCol.key)}
                        onChange={(e) =>
                          setAllChecked(menuCol.key, e.target.checked)
                        }
                      />
                      ...
                      <div className="col-menu-list">
                        {shownValues.map((v) => (
                          <label key={v} className="col-menu-item">
                            <input
                              type="checkbox"
                              checked={isChecked(menuCol.key, v)}
