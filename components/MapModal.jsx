...
export default function MapModal({ onPickAsset, onOpenDefineDevice, onClose, defaults = {} }) {
  ...
  // ✅✅ Esc فقط وقتی DwgBrowser باز نیست، خود MapModal را می‌بندد
  useEffect(() => {
    const h = (e) => {
  ...
  const applyLayerAnswers = async () => {
    for (const layer of unknown) {
      ...
      if (a === 'new') {
        ...
        await fetch('/api/maps/layer-rule', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ layerName: layer, deviceType: '__IGNORE__', isNewType: true }) });
      } else {
  ...
  useEffect(() => {
    ...
    const onClick = async (e) => {
      ...
      const layer = tag ? (tag.Layer || tag.layer || '') : (gLayer ? gLayer.getAttribute('data-layer') : (el.getAttribute('data-layer') || ''));
      const r = ruleForLayer(rules, layer);
      const inferredType = r ? r.DeviceType : (deviceType || '');
      if (inferredType === '__IGNORE__') { say('این لایه «نادیده» است؛ ثبت دستگاه انجام نمی‌شود.', 3000); return; }
