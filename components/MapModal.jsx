...
const parseMapName = (name) => {
  ...
  if (m3) {
    out.building =
      BUILDING_ALIASES[m3[1]] || BUILDING_ALIASES[m3[1].toLowerCase()] || m3[1];
  ...
  out.building =
    BUILDING_ALIASES[bRaw.toLowerCase()] || BUILDING_ALIASES[bRaw] || bRaw;
  return out;
};
...
const parseMapInfo = (full) => {
  ...
  for (const s of segs) {
    ...
    if (!out.building) {
      ...
      else if (BUILDING_ALIASES[t.toLowerCase()])
        out.building = BUILDING_ALIASES[t.toLowerCase()];
    }
    if (!out.building || !out.block || !out.floor) {
      const pn = parseMapName(t);
      if (!out.building && pn.building && pn.building !== t)
        out.building = pn.building;
      if (!out.block && pn.block) out.block = pn.block;
      if (!out.floor && pn.floor) out.floor = pn.floor;
    }
  }
  if (!out.building && /مرکزی|Markazi/i.test(String(full)))
    out.building = "مرکزی";
  if (!out.building && /سردخانه|Sardkhaneh/i.test(String(full)))
    out.building = "سردخانه";
  if (!out.block) {
...
const ruleForLayer = (rules, L) => {
  ...
  if (r) return r;
  const stripped = String(L).replace(
    /[-_ ]?(Text|Num|Numbers|Nums|Tag|Label)s?$/i,
    "",
  );
    ...
    <div className={`mb-2 ${color} rounded`}>
      ...
      >
        <span>
          {title}
          {badge ? (
            <span className="mr-2 bg-white/70 rounded px-1 text-xs">
              {badge}
            </span>
          ) : null}
        </span>
...
export default function MapModal({
  ...
  const load = async (b = building, bl = block, f = floor) => {
    ...
    try {
      ...
      setCenter(
        d.map && d.map.CenterX != null
          ? { x: d.map.CenterX, y: d.map.CenterY }
          : null,
      );
      let svgOk = false;
      ...
      if (!svgOk)
        say(
          "فایل SVG نقشه یافت نشد (404)؛ ابتدا «🔄 تبدیل مجدد» یا «⚠ همگام‌سازی» را بزنید.",
          8000,
        );
    } catch (e) {
  ...
  const deactivateOrphans = async () => {
    ...
    setOrphan([]);
    alert(
      "دستگاه‌های حذف‌شده از نقشه، «ناموجود» علامت‌گذاری شدند (سابقه حفظ شد).",
    );
  };
  ...
  useEffect(() => {
    ...
    box.querySelectorAll("g[data-layer]").forEach((g) => {
      ...
      g.style.display =
        isB || (r && (!deviceType || r.DeviceType === deviceType))
          ? ""
          : "none";
    });
...
if (inferredType === '__IGNORE__') { say('این لایه «نادیده» است؛ ثبت دستگاه انجام نمی‌شود.', 3000); return; }      const numMatch = txt.match(/^\s*-?\d+(?:\.\d+)?\s*$/);
      ...
      try {
        ...
        if (d.found) {
          ...
          alert(
            `دستگاه «${txt}» در دیتابیس نیست و مودال تعریف دستگاه متصل نشده است.`,
          );
        }
      } catch (err) {
  ...
  return (
    ...
    >
      <div className="bg-[#CCE6DF] rounded-lg shadow-2xl w-[1100px] max-w-full max-h-[92vh] overflow-y-auto p-4">
        ...
        {statusMsg && (
          <div className="mb-2 bg-teal-100 text-teal-900 rounded px-2 py-1 text-sm font-bold">
            {statusMsg}
          </div>
        )}
            ...
            {unknown.map((l) => (
              <div key={l} className="flex gap-2 items-center mt-2">
                ...
                <select
                  ...
                  value={answers[l] || ""}
                  onChange={(e) =>
                    setAnswers({ ...answers, [l]: e.target.value })
                  }
                >
                  <option value="">(انتخاب)</option>
                  {deviceTypes.map((t) => (
                ...
                {answers[l] === "__new__" && (
                  <input
                    ...
                    value={newNames[l] || ""}
                    onChange={(e) =>
                      setNewNames({ ...newNames, [l]: e.target.value })
                    }
                  />
                )}
              </div>
        ...
        {newOnMap.length > 0 && (
          <Sec
            ...
            open={open.newOnMap}
            onToggle={toggleSec}
            title="🆕 دستگاه‌های جدید روی نقشه (بدون ثبت)"
            badge={newOnMap.length}
          >
              ...
              {newOnMap.map((t) => (
                <label key={t.text} className="block text-sm mt-1">
                  <input
                    ...
                    onChange={(e) =>
                      setChecked({ ...checked, [t.text]: e.target.checked })
                    }
                  />
                  {t.text}{" "}
        ...
        {orphan.length > 0 && (
          <Sec
            ...
            open={open.orphan}
            onToggle={toggleSec}
            title="⚠ در دیتابیس هستند ولی روی نقشه نیستند"
            badge={orphan.length}
            color="bg-[#FC7470]/30"
          >
            {orphan.map((o) => (
        ...
        .mapzoom-wrap svg line,.mapzoom-wrap svg path,.mapzoom-wrap svg polyline,.mapzoom-wrap svg polygon,.mapzoom-wrap svg circle,.mapzoom-wrap svg ellipse{stroke:#1f2937 !important;vector-effect:non-scaling-stroke;}            .mapzoom-wrap svg text{font-weight:600;}
          ...
          <div className="absolute top-2 right-2 z-20 bg-white/90 rounded shadow px-2 py-1 text-xs font-bold">
            ...
            {centerMode && (
              <span className="mr-2 text-orange-600">کلیک روی نقشه</span>
            )}
          </div>
          <div
            ...
            dangerouslySetInnerHTML={{
              __html:
                svgText ||
                '<div style="padding:40px;text-align:center">نقشه‌ای بارگذاری نشده</div>',
            }}
          />
        </div>
        <div className="text-[11px] text-gray-600 mt-1">
          چرخ ماوس = زوم | کشیدن = جابه‌جایی | کلیک روی برچسب قرمز = انتخاب/ثبت
          | 🎯 نقطهٔ قرمز = مرکز نقشه
        </div>
      </div>
      {showBrowse && (
        <DwgBrowser
